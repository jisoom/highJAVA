...
public class MultiChatServer {
	...
	public MultiChatServer() {
		//동기화 처리가 가능하도록 Map 객체 생성 //한 스레드만 접속할 수 있도록
	...
	class ServerReceiver extends Thread {
		...
		public void whisper(String msg) {
			String[] nameMsg = msg.split(" ");
			
			try {
				DataOutputStream reDos = new DataOutputStream(socket.getOutputStream());
				
				if(!nameMsg[0].equals("/w")) {
					reDos.writeUTF("명령어를 잘못 입력하였습니다. (/w 닉네임)");
					return;
				}
				if(!clients.keySet().contains(nameMsg[1])) {
					reDos.writeUTF("존재하지 않는 닉네임입니다.");
					return;
				}
				
				String whisperMsg ="";
				for(int i=2; i < nameMsg.length; i++) {
					whisperMsg += nameMsg[i] + " ";
				}
				DataOutputStream whisperDos = new DataOutputStream(clients.get(nameMsg[1]).getOutputStream());
				whisperDos.writeUTF("귓속말 >>[" + name + "]" + whisperMsg);
