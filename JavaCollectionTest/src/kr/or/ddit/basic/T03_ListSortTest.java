...
public class T03_ListSortTest {
	 ...
	 * -보통 객체 자체에 정렬 기능을 넣기 위해서는 Comparable을 구현하고 정렬 기준을 별도로 구현하고 싶을 때에는 Comparator를 구현하여 사용하면 된다.
	 * -Comparable 에서는 compareTo()메서드를 구현해야하고,
	...
	public static void main(String[] args) {
		...
		//정렬 방식을 변경하려면 정렬 방식을 결정하는 객체를 만들어서
		//Collections.sort()메서드에 인수로 넘겨주면 된다.
		
		Collections.sort(list); //오름차순 정렬하기
		System.out.println("정렬 후 : " + list);//기본 오름차순 정렬이므로 이름순으로 오름차순 정렬됨
...
class Desc implements Comparator<String>{
	...
	public int compare(String str1, String str2) {
		return str1.compareTo(str2)*-1; //오름차순 ==> *1, 내림차순 하고 싶을 때는 *-1 ==> 반대의 정렬이 됨
	}
}