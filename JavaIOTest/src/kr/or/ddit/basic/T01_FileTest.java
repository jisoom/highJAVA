...
public class T01_FileTest {
	public static void main(String[] args) throws IOException {
		...
		System.out.println("파일 여부 : " + file.isFile()); //이게 파일이고 경로에 존재하냐 ? 
		System.out.println("디렉토리(폴더) 여부: " + file.isDirectory()); //이게 폴더고 경로에 존재하냐?
		System.out.println("----------------------------------------");
		...
		System.out.println("절대 경로 : " + file4.getAbsolutePath()); //d:이런거 붙여서
		//상대 경로 : 현재 위치를 기준으로 , 절대 경로: 절대적인 경로
		System.out.println("경로 : " + file4.getPath()); //적어준 문자열 그대로
		System.out.println("표준 경로 : " + file4.getCanonicalPath()); //계산된 경로 (예외 던짐)
