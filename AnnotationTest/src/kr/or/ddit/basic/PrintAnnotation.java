...
/*
	Annotation에 대하여...
	프로그램 소스 코드 안에 다른 프로그램을 위한 정보를 미리 약속된 형식으로 포함시킨 것(JDK1.5부터 지원함)
	주석처럼 프로그래밍 언어에 영향을 미치지 않으면서도 다른 프로그램에게 유용한 정보를 제공함.
	
	종류 : 1. 표준 애너테이션(주로 컴파일러에게 유용한 정보를 제공하기 위한 에너테이션)
		 2. 메타 에너테이션(애너 테이션을 위한 애너테이션, 즉 애너테이션을 위한 정의할 때 사용하는 애너테이션)
...
@Retention(RetentionPolicy.RUNTIME) //유지기간 지정함.(기본 값: CLASS) //가장 오래까지 살아있는 것 : RUNTIME (거의 RUNTIME이 많음)
