...
public class T10_ThreadStateTest {
	 ...
	 * -WAITING, TIMED_WAITING : 스레드의 작업이 종료되지는 않았지만 실행가능하지 않은 일시정지 상태(UNRINNABLE)
...
class StatePrintThread extends Thread {
	...
	public void run() {
		while(true) {
			...
			System.out.println("타겟 스레드의 상태 값 : " + state);
...
class TargetThread extends Thread {
	...
	public void run() {
		...
		try {
			Thread.sleep(1500); // =>StatePrintThread스레드가 Thread.sleep(500); 이기 때문에 TIMED_WAITING 상태가 3번 이루어짐
		} catch (InterruptedException e) {
			e.printStackTrace();
		}
